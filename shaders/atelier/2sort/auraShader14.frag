...
const float square_columns = 13.0;
...
void main() {
  float square_columns = square_columns * 0.1;
  ...
  vec2 uv = tan(gl_FragCoord.y*log(u_time)/gl_FragCoord.x) * square_columns * gl_FragCoord.xy / (min(u_resolution.x, u_resolution.y));

  uv -= vec2(CENTER);
  vec2 floorUV = floor(uv);
  float randomF =randomFromFloat(uv.y, DEFAULT_RANDOM_FROM_FLOAT_PARAM);
  uv = rotate2d( -((4.0*floorUV.y*tan(0.5*u_time))/4.0)* (tan(uv.y)*uv.y)*PI) * uv;
  uv += vec2(CENTER);
  ...
  float centeringFactor = (length(m)*log(abs(tan(u_time/40.0)/4.0)));
