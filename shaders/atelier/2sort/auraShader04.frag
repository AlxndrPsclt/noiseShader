...
const float SQUARE_COLUMNS = 1.0;
...
void main() {
  ...
  uv -= vec2(CENTER);
  // rotate the space
  uv = rotate2d( (cos(u_time/100.0)/2.0)*PI ) * uv;
  // move it back to the original place
  uv += vec2(CENTER);

  vec3 draw = palette(uv.y/uv.x);
  ...
  vec3 color = tan(vec3(showCell*draw.x*0.3, showCell*draw.y*0.3, showCell*draw.z*0.3));
