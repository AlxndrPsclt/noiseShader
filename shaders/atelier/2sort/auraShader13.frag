...
void main() {
  float square_columns = square_columns * 0.2;
  ...
  vec2 uv = sin(gl_FragCoord.x) * square_columns * gl_FragCoord.xy / (min(u_resolution.x, u_resolution.y));

  uv -= vec2(CENTER);
  float randomF =randomFromFloat(uv.y, DEFAULT_RANDOM_FROM_FLOAT_PARAM);
  uv = rotate2d( ((2.0-sin(u_time/10.0))/4.0)* (tan(uv.x)*uv.y)*PI) * uv;
  uv += vec2(CENTER);
  ...
  float centeringFactor = (0.3-log(abs(tan(u_time/10.0)/4.0)));
