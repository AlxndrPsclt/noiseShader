...
//
    vec3 layer00 = texture2D(u_texture_0, vec2(1.0-uv.x,uv.y)).xyz;
    ...
    color += float(uv.x < (rand(uv.y*u_time)/100.0 + abs(cos(1.0+abs(cos(u_time/3.0)))/6.0))+float(uv.x >0.12+abs(0.5+2.0*sin(sin(u_time/3.0)))/6.0))*vec3(0.33,0.77,0.9);
    color += float(uv.y < (rand(uv.x*u_time)/100.0 + abs(cos(1.0+abs(sin(u_time/3.0)))/6.0))+float(uv.y >0.73*(1.0+sin(u_time/2.0)/4.0)+abs(0.5+2.0*sin(cos(u_time/6.0)))/6.0))*vec3(0.63,0.17,0.3);

    uv -= vec2(noise(u_time),0.5);
    uv=rotate2d(noise(u_time)*PI/4.0)*uv;
    uv += vec2(0.5*noise(u_time),0.5);

    color += float(uv.x < (rand(uv.x*u_time/3.0)/100.0 + abs(cos(1.7+abs(cos(u_time/6.0)))/6.0))+float(uv.x >0.12+abs(0.5+2.0*sin(sin(u_time/3.0)))/6.0))*vec3(0.33,0.77,0.9);
    ...
    gl_FragColor = 1.0/vec4(log(0.1*u_time*color), 1.0);
}
