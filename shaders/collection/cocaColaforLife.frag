...
//    //uv/=0.77*noise(u_time/10.0);
    ...
    //color *= (0.8);
      ...
      float shift = 0.0;
      // Apply threshold
      if(differenceFactor > 0.88) {  // Adjust the threshold value as needed
    ...
    for(float i = 1.0; i < 100.0; i += 1.0) {
      float lineSDF = sdLine(uv, vec2((0.02+0.05*differenceFactor)*i - 1.0, 0.0), vec2((0.02+0.05*differenceFactor)*i-1.0, 1.0));
