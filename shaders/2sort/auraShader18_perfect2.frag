...
void main() {
  ...
  uv -= vec2(CENTER);
  float randomF =randomFromFloat(uv.x, DEFAULT_RANDOM_FROM_FLOAT_PARAM);

  //uv = rotate2d( +((floorUV.x+floorUV.y + 1.0) *m.x* randomF*tan(0.15*u_time))* (tan(m.y*uv.x*uv.y))*PI) * uv;
  uv = rotate2d( +((floorUV.x+floorUV.y + 2.0) *2.9* randomF*tan(0.15*u_time))* (tan(uv.x*uv.y*0.001))*PI) * uv;
  //uv = rotate2d( -(PI)) * uv;
  uv += vec2(CENTER);
  ...
  float centeringFactor = (abs(tan(u_time/40.0)/uv.x));
  //float centeringFactor = 1.0;
